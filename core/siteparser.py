...
def get_photos_url(url_without_photos):
    return urlparse.urljoin(config.base_url, os.path.join(config.photos, url_without_photos))


def get_alphas_urls():
    url = os.path.join(config.base_url, config.photos)
...
def get_alphas_local():
    with open(os.path.join(config.settings_dir, 'alphas_list.txt')) as f:
        for line in f:
            yield urlparse.urljoin(config.base_url, line.strip())
...
def get_page_url(the_place_id, page_num):
    ...
    return urlparse.urljoin(config.base_url, url)
...
def get_pages_count(celeb):
    ...
    page = config.utls.getPage(celeb.url)
...
def get_icons(celeb, page):
    ...
    page = config.utls.getPage(celebs_page)
    soup = bs4.BeautifulSoup(page)
    tags = soup.select('.pic_box img.pic')
    for t in tags:
        t = get_photos_url(t.get('src'))
        icon = ThePlaceImage(t, celeb)
        yield icon, len(tags)
...
def get_celebs_for_alpha(alpha_page_url):
    page = config.utls.getPage(alpha_page_url)
    soup = bs4.BeautifulSoup(page)
    tags = soup.select(".m-box a.t")
